...
const Config = z.object({
  ...
  searchPath: z.string().default(DEFAULT_SEARCH_PATH),
  fetchPath: z.string().default(DEFAULT_FETCH_PATH),
  /** Cap on the per-source snippet length (search). */
  snippetMax: z.number().step(1).min(1).default(DEFAULT_SNIPPET_MAX),
...
function OllamaSearchProvider(resolveOptions) {
  this.id = 'ollama';
  this.resolveOptions = resolveOptions;
}

OllamaSearchProvider.prototype.available = function () {
  const o = this.resolveOptions();
  return typeof o.baseURL === 'string' && o.baseURL.length > 0;
};

OllamaSearchProvider.prototype.search = async function (request, signal) {
  const o = this.resolveOptions();
  ...
  try {
    ...
    if (signal?.aborted === true) throw new WebError('Ollama web search aborted', 'WEB_ABORTED', { cause: error });
  ...
  try {
    ...
    throw new WebError('Ollama web search returned an unprocessable response body', 'WEB_PROVIDER_ERROR', { cause: error });
  }
  ...
  for (const item of items) {
    ...
    const snippet = raw === undefined
      ? undefined
      : raw.length > o.snippetMax
        ? `${raw.slice(0, o.snippetMax)}…`
        : raw;
    sources.push({
      url,
...
function OllamaFetchProvider(resolveOptions) {
  this.id = 'ollama';
  this.resolveOptions = resolveOptions;
}

OllamaFetchProvider.prototype.available = function () {
  const o = this.resolveOptions();
  return typeof o.baseURL === 'string' && o.baseURL.length > 0;
};

OllamaFetchProvider.prototype.fetch = async function (request, signal) {
  const o = this.resolveOptions();
  const apiKey = await o.resolveApiKey();
  const endpoint = `${o.baseURL.replace(/\/+$/, '')}${o.fetchPath}`;
  const payload = { url: request.url };
  o.recordRequest?.({ endpoint, body: payload });
  const merged = abortSignalAny([signal, AbortSignal.timeout(o.fetchTimeoutMs)]);
  let response;
  try {
    response = await fetch(endpoint, {
      method: 'POST',
    ...
    if (signal?.aborted === true) throw new WebError('Ollama web fetch aborted', 'WEB_ABORTED', { cause: error });
    if (error?.name === 'TimeoutError') throw new WebError(`Ollama web fetch timed out after ${o.fetchTimeoutMs}ms`, 'WEB_PROVIDER_ERROR', { cause: error });
    throw new WebError(`Ollama web fetch request failed: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error });
  }
  if (!response.ok) {
  ...
  try {
    ...
    throw new WebError('Ollama web fetch returned an unprocessable response body', 'WEB_PROVIDER_ERROR', { cause: error });
  }
