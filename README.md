...
| 以 bundle 分发（`dsh.bundle.patch`），可被 `dsh plugin` / Web Plugins 页安装管理 | ✅ v0.1.9 起（host 与 client 两个包都是） |
| 浏览器半注册键值插槽 `plugins.row.config`（key = `组合包名#行id`）才换来行上的「配置」入口 | ✅ v0.1.11 起 |
